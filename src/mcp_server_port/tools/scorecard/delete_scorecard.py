...
class DeleteScorecardTool(Tool):
    ...
    async def delete_scorecard(self, props: DeleteScorecardToolSchema) -> Dict[str, Any]:
        ...
        try:
            await self.port_client.get_scorecard_details(scorecard_id, blueprint_id)
        ...
        if result:
            return DeleteScorecardToolResponse(success=True, message=f"✅ Successfully deleted scorecard '{scorecard_id}' from blueprint '{blueprint_id}'").dict()
        else:
            return DeleteScorecardToolResponse(success=False, message=f"❌ Failed to delete scorecard '{scorecard_id}'. No error was reported, but the operation may not have succeeded.").dict()
