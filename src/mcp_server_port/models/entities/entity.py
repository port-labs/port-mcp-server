...
class SearchRuleEquals(BaseModel):
    ...
    value: Union[str, int] = Field(..., description="The value of the search rule")
...
class SearchRuleQuery(BaseModel):
    ...
    rules: List[Union[SearchRuleEquals, SearchRuleIn]] = Field(..., description="The conditions of the search query")
...
class CommonEntity(BaseModel):
    ...
    identifier: str | Union[SearchQuery, SearchRuleIn, SearchRuleEquals] | SkipJsonSchema[None] = Field(..., description="The identifier of the new entity. New entities must match the regex pattern: ^[A-Za-z0-9@_.:\\/=-]+$")
    ...
    properties: Dict[str, Any] | SkipJsonSchema[None] = Field(None, description='An object containing the properties of the new entity, in "key":"value" pairs where the key is the property\'s identifier, and the value is its value.')
    relations: Dict[str, Any] | SkipJsonSchema[None] = Field(None, description="""An object containing the relations of the new entity, in "key":"value" pairs where the key is the relation's identifier, and the value is the related entity's identifier. You can also use a search query to define relations based on a property of the related entity."""),
