...
class InvokeAIAGentTool(Tool):
    ...
    async def invoke_ai_agent(self, props: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while attempt < max_attempts:
            ...
            if status.status.lower() in ["completed", "failed", "error"]:
                ...
                return InvokeAIAGentToolResponse(invocation_id=identifier, invocation_status=status.status, message="").model_dump()
        ...
        return InvokeAIAGentToolResponse(invocation_id=identifier, invocation_status="timed_out", message="⏳ Operation timed out. You can check the status later with identifier: {identifier}").model_dump()