...
from src.server.models import PortAgentResponse, Blueprint, Scorecard, Entity, CreateBlueprint
...
class PortClient:
    ...
    def __init__(self, client_id: Optional[str] = None, client_secret: Optional[str] = None, region: str = "EU", base_url: str = config.PORT_API_BASE):
    ...
    async def delete_entity(self, blueprint_identifier: str, entity_identifier: str,delete_dependents: bool = False) -> bool:
        return await self.wrap_request(lambda: self.entities.delete_entity(blueprint_identifier, entity_identifier,delete_dependents))
    
    async def get_scorecard(self, scorecard_id: str, blueprint_id: str = None) -> Scorecard:
