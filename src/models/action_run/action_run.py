...
class ActionRunReference(BaseModel):
    ...
    deleted: bool | SkipJsonSchema[None] = Field(
        None, description="Whether the referenced item is deleted"
    )
...
class ActionRunApproval(BaseModel):
    ...
    user_id: str | SkipJsonSchema[None] = Field(
        None, description="User ID who approved", alias="userId"
    )
...
class ActionRunPayload(BaseModel):
    ...
    synchronized: bool | SkipJsonSchema[None] = Field(
        None, description="Whether execution is synchronized"
    )
...
class ActionRun(BaseModel):
    ...
    blueprint: ActionRunReference | SkipJsonSchema[None] = Field(
        None, description="Reference to the blueprint"
    )
    entity: ActionRunReference | SkipJsonSchema[None] = Field(
        None, description="Reference to the entity"
    )
    action: ActionRunReference = Field(..., description="Reference to the action")
    properties: dict[str, Any] = Field(
        default_factory=dict, description="Input properties provided for this run"
    )
    ended_at: str | SkipJsonSchema[None] = Field(
        None, description="When the run ended", alias="endedAt"
    )
    source: dict[str, Any] | SkipJsonSchema[None] = Field(
        None, description="Source information for the run"
    )
    ...
    status_label: str | SkipJsonSchema[None] = Field(
        None, description="Human-readable status label", alias="statusLabel"
    )
    ...
    approval: ActionRunApproval | SkipJsonSchema[None] = Field(
        None, description="Approval information"
    )
    payload: ActionRunPayload | SkipJsonSchema[None] = Field(
        None, description="Payload configuration"
    )
    response: list[str] | SkipJsonSchema[None] = Field(
        None, description="Response from the action execution"
    )
    created_by: str | SkipJsonSchema[None] = Field(
        None, description="User who created the run", alias="createdBy"
    )
    updated_by: str | SkipJsonSchema[None] = Field(
        None, description="User who last updated the run", alias="updatedBy"
    )
    created_at: str | SkipJsonSchema[None] = Field(
        None, description="Creation timestamp", alias="createdAt"
    )
    updated_at: str | SkipJsonSchema[None] = Field(
