...
class DeleteEntityToolSchema(BaseModel):
    ...
    blueprint_identifier: str = Field(
        ..., description="The identifier of the blueprint to delete the entity for"
    )
    ...
    run_id: str | SkipJsonSchema[None] = Field(
        default=None, description="The run_id of the action to delete the entity for"
    )
...
class DeleteEntityTool(Tool[DeleteEntityToolSchema]):
    ...
    async def delete_entity(self, props: DeleteEntityToolSchema) -> dict[str, Any]:
        ...
        result = await self.port_client.delete_entity(
